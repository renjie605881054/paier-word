...
public class ThreeDes {
	...
	// 向量
	private final static String iv = "treasure";
	// 加解密统一使用的编码方式
	private final static String encoding = "utf-8";

	public static String generatorSecretKey(){
		StringBuffer key = new StringBuffer();
		char[] chars = { '2', '3', '4', '5', '6','7', '8', '9', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I' ,'J','K','L','M','N','P','Q','R','S','T','V',
				'W','X','Y','Z','a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i' ,'j','k','m','n','p','q','r','s','t','v','w','x','y','z'};
		for( int i = 0 ; i < 24 ; i ++ ){
			int nowKey = ThreadLocalRandom.current().nextInt(chars.length);
			key.append(chars[nowKey]);
		}
		return key.toString();
	}
...
//		String encryptText = ThreeDes.encode(plainText, key);
//		System.out.println(key + "--" + encryptText);
//		System.out.println(ThreeDes.decode(encryptText,key));
	}
}